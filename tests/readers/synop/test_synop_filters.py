...
def test_synop_filter_wigos():
    df = pdbufr.read_bufr(
        ...
        reader="synop",
        columns="station",
        filters={"sid": "0-705-0-1931"},
    )

    df = df.replace(np.nan, None)

    assert df.shape == (1, 5)
    assert df["sid"].iloc[0] == "0-705-0-1931"
...
def test_synop_filter_wmoid_uc():
    df = pdbufr.read_bufr(
        ...
        reader="synop",
        columns="station",
        filters={"sid": [1308, 1084]},
    )

    df = df.replace(np.nan, None)

    assert df.shape == (2, 5)
    assert df["sid"].iloc[0] == 1084
    assert np.isclose(df["elevation"].iloc[0], 27)

    assert df["sid"].iloc[1] == 1308
...
def test_synop_filter_new_1a():
    df = pdbufr.read_bufr(
        ...
        reader="synop",
        columns="station",
        filters={"sid": [11766, 56257], "t2m": slice(0, 275), "t2m_level": (0, 1.9)},
    )
...
def test_synop_filter_uc_1():
    df = pdbufr.read_bufr(
        ...
        reader="synop",
        columns=["station", "t2m"],
        filters={"sid": [1308, 1084], "t2m": slice(0, 275)},
    )
